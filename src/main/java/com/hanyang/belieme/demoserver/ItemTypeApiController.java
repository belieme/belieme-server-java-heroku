...
public class ItemTypeApiController {
    ...
    @GetMapping("/{id}")
    public ResponseWrapper<ItemType> getItem(@PathVariable UUID id) {
    ...
    public ResponseWrapper<ArrayList<ItemType>> updateItem(@RequestBody ItemType item){
        if(item.getId() == null || item.getName() == null || item.getEmoji() == null) { // id가 0으로 자동 생성 될 수 있을까? 그리고 typeId 안쓰면 어차피 뒤에서 걸리는데 필요할까?
            return new ResponseWrapper<>(ResponseHeader.LACK_OF_REQUEST_BODY_EXCEPTION, null);
        }
    ...
    public ResponseWrapper<Void> deactivateItem(@PathVariable UUID id) {
